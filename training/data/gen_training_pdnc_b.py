...
def read_quotes(root_folder, mappers, novel):
	...
	for _, row in quotedf.iterrows():
		...
		try:
			...
			quoteinf.append((qId, st, et, eid, qType, qtt.strip()))
		except KeyError:
...
def get_context_data(tokens, quotes, corefs, fulltoks, idd, train_ids, window=30):
	# fifty tokens on either side of quotation, excluding other quotations.
	end_quotes={}
	in_quotes=np.zeros(len(tokens))
	for idx, (qid, q_start, q_end, eid, _, qtt) in enumerate(quotes):
	...
	for qid, start_tok, end_tok, q_eid, q_type, q_text in quotes:
		q_toks = []

		for i in range(start_tok, end_tok):
			q_toks.append(tokens[i])
		...
		for i in range(start, end):
			...
			if i == end_tok:
				...
				(oqid, q_start, q_end, eid, _, _)=quotes[end_quotes[i]] #speaker of the other quotations in context
		...
		diff = 500 - len(wp_toks)
		qp_toks = tokenizer.tokenize(' '.join(q_toks))
		use_qp = qp_toks[:diff]
		use_qt = " ".join(qp_toks)
		...
		if len(cands) > 0:
			...
			all_quotes.append([idd, qid, q_eid, index, labels, text, use_qt])
	...
	if writeFolder is not None:
		for split, data in zip(['train', 'dev', 'test'], [train_data, val_data, test_data]):
			...
			with open(os.path.join(writeFolder, 'quotes.'+split+'.txt'), 'w') as f:
				# writer = csv.writer(f)
				for idd, qid, q_eid, index, labels, text, qtext in data:
					print("%s\t%s\t%s\t%s\t%s\t%s\t%s" % (idd, qid, q_eid, index, json.dumps(labels), text, qtext), file=f)
