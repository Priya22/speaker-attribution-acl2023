...
def get_batches(model, sentences, max_batch, tagset, training=True):
	...
	for sentence in sentences:
		...
		for word in sentence:
			...
			# if word[0].lower().startswith("[cap]"):
			# print(word[0], toks)
			length+=len(toks)
	...
	for sentence in sentences:
		...
		if training:
			...
			newlabels=model.compress([layered_labels1, layered_labels2, layered_labels3], rev_tagset)
			indices=model.get_index(newlabels, rev_tagset)
